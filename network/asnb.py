...
class ScalePoolingModule(nn.Module):
    ...
    def forward(self, feats):
        ...
        if self.dimension == 1:
            n, c, s1 = feats.size()
            output = F.adaptive_avg_pool2d(feats, output_size=int(s1*self.scale))
        elif self.dimension == 2:
            n, c, s1, s2 = feats.size()
            output = F.adaptive_avg_pool2d(feats, output_size=(int(s1*self.scale), int(s2*self.scale)))
        elif self.dimension == 3:
            n, c, s1, s2, s3 = feats.size()
            output = F.adaptive_avg_pool2d(feats, output_size=(int(s1*self.scale), int(s2*self.scale), int(s3*self.scale)))
...
class _SelfAttentionBlock(nn.Module):
    ...
    def __init__(self, low_in_channels, high_in_channels, key_channels, value_channels, out_channels=None, scale=1, norm_type=None, attn_scale=0.25):
        ...
        self.spool = ScalePoolingModule(attn_scale)
    ...
    def forward(self, low_feats, high_feats):
        ...
        value = self.spool(self.f_value(low_feats))
        ...
        key = self.spool(key)  # .view(batch_size, self.key_channels, -1)
